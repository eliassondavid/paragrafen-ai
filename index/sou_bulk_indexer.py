"""Bulk-index SOU raw documents directly into ChromaDB."""
...
def extract_part(filename: str, dok_id: str) -> int | None:
    """Extract multi-part suffix from filename or dok_id."""
    match = re.search(r"_d(\d+)", Path(filename).stem)
    if match:
        return int(match.group(1))
    match = re.search(r"d(\d+)$", dok_id or "")
    if match:
        return int(match.group(1))
    return None


def parse_beteckning(raw: dict[str, Any], filepath: Path) -> tuple[str, int]:
    """Return (år, nr) as (str, int) or raise SkipDocument."""
    beteckning = str(raw.get("beteckning") or "").strip()

    match = re.match(r"SOU\s+(\d{4}):(\d+)", beteckning)
    if match:
        return match.group(1), int(match.group(2))

    file_match = re.match(r"(?:SOU|sou)_(\d{4})_0*(\d+)", filepath.stem)
    if file_match:
        return file_match.group(1), int(file_match.group(2))

    raise SkipDocument(f"Cannot parse beteckning: {beteckning!r} (fil: {filepath.name})")


def build_chunk_id(år: str, nr: int, part: int | None, chunk_index: int) -> str:
    """Build the mandated SOU chunk namespace."""
    part_suffix = f"_d{part}" if part else ""
    return f"sou::{år}_{nr}{part_suffix}_chunk_{chunk_index:03d}"


def main(argv: list[str] | None = None) -> int:
    """CLI entrypoint."""
    parser = argparse.ArgumentParser(description="Bulk-indexera SOU till ChromaDB")
    ...
    parser.add_argument(
        ...
        action="append",
        default=None,
        help="Indexera endast angivna råfiler (kan anges flera gånger)",
    )
...
class SouBulkIndexer:
    """Read raw SOU JSON, parse, chunk, embed and upsert to Chroma."""
    ...
    def __init__(
        ...
        *,
        ...
        self.sou_rank = load_sou_rank(self.rank_config_path)
    ...
    def run(
        ...
        *,
        ...
        """Execute the full pipeline over all raw SOU files."""
        raw_files = collect_raw_files(self.raw_dir)
        if include_files:
            include_names = set(include_files)
            raw_files = [path for path in raw_files if path.name in include_names]
    ...
    def prepare_document(
        ...
        *,
        ...
        """Prepare one raw SOU document for indexing."""
        ...
        år, nr = parse_beteckning(raw, raw_path)
        part = extract_part(raw_path.name, str(raw.get("dok_id") or ""))

        sections = parse_sou_html(html_content, str(raw.get("dok_id") or ""))
        if not sections:
        ...
        beteckning = build_default_beteckning(år, nr)
        canonical_citation = build_canonical_citation(år, nr, fallback=beteckning)
        ...
        for chunk_index, (chunk, page_range) in enumerate(zip(chunked_sections, page_ranges)):
            ...
            chunk_id = build_chunk_id(år, nr, part, chunk_index)
...
def collect_raw_files(raw_dir: Path) -> list[Path]:
    """Collect raw and curated SOU files."""
...
def load_sou_rank(config_path: Path) -> int:
    """Load forarbete_rank for SOU from YAML."""
    with config_path.open("r", encoding="utf-8") as fh:
        rank_config = yaml.safe_load(fh) or {}
    rank = rank_config.get("forarbete_types", {}).get("sou", {}).get("rank")
    if not isinstance(rank, int):
        raise ValueError("forarbete_rank för sou saknas eller är inte ett heltal.")
    return rank


def build_default_beteckning(år: str, nr: int) -> str:
    """Build fallback SOU label."""
    if år and nr > 0:
        return f"SOU {år}:{nr}"
    if år:
        return f"SOU {år}"
...
def build_canonical_citation(år: str, nr: int, *, fallback: str) -> str:
    """Build canonical SOU citation."""
    if år and nr > 0:
        return f"SOU {år}:{nr}"
    return fallback
