"""Bulk-index rskr raw documents directly into ChromaDB."""
...
INPUT_DIR = "data/raw/rskr"
SCHEMA_VERSION = "v0.18"
LICENSE = "public_domain"
SOURCE_TYPE = "forarbete"
DOCUMENT_SUBTYPE = "rskr"
AUTHORITY_LEVEL = "preparatory"
PAGE_FIELD_WARNING = (
    "Section/ChunkedSection saknar page_start/page_end; rskr-indexern bar darfor sidinformation separat."
)
...
def extract_nested_dokument(raw: dict[str, Any]) -> dict[str, Any]:
    """Return nested dokument payload when available."""
...
def normalize_beteckning(beteckning: str, rm: str, titel: str) -> str:
    """Normalize to the canonical rskr citation style."""
    value = (beteckning or "").strip()
    title_value = (titel or "").strip()

    if re.search(r"(?i)\brskr\b", value):
        return value

    match = re.search(r"(?i)riksdagsskrivelse\s+(\d{4}(?:/\d{2,4})?)\s*:\s*(\d+)", title_value)
    if match:
        return f"rskr {match.group(1)}:{match.group(2)}"
...
def extract_metadata(raw: dict[str, Any]) -> dict[str, Any]:
    """Extract metadata from dataset-nested or flat fetched rskr files."""
    dokument = extract_nested_dokument(raw)
    if dokument:
        html_content = str(dokument.get("html") or "")
        dok_id = str(dokument.get("dok_id") or raw.get("dok_id") or "").strip()
        rm = str(dokument.get("rm") or "").strip()
        raw_beteckning = str(dokument.get("beteckning") or "").strip()
        titel = str(dokument.get("titel") or "").strip()
        datum = str(dokument.get("datum") or "").strip()[:10]
        ...
        dok_id = str(raw.get("dok_id") or "").strip()
        rm = str(raw.get("rm") or "").strip()
        raw_beteckning = str(raw.get("beteckning") or "").strip()
        titel = str(raw.get("titel") or "").strip()
        datum = str(raw.get("datum") or "").strip()[:10]
        organ = str(raw.get("organ") or "").strip()
        source_url = str(raw.get("source_url") or f"https://data.riksdagen.se/dokument/{dok_id}").strip()

    beteckning = normalize_beteckning(raw_beteckning, rm, titel)
    nummer = 0
    match = re.search(r":(\d+)\s*$", beteckning.strip())
    if match:
        nummer = int(match.group(1))
    elif raw_beteckning.isdigit():
        nummer = int(raw_beteckning)
...
def build_chunk_id(meta: dict[str, Any], chunk_index: int) -> str:
    """Build the mandated rskr namespace."""
    rm = str(meta.get("rm") or "").strip()
    nummer = coerce_int(meta.get("nummer"))
    dok_id = str(meta.get("dok_id") or "").strip().lower()
    if rm and nummer > 0:
        rm_norm = rm.replace("/", "-")
        return f"rskr::{rm_norm}_{nummer}_chunk_{chunk_index:03d}"
    return f"rskr::{dok_id}_chunk_{chunk_index:03d}"
...
def load_forarbete_rank(config_path: Path) -> int:
    """Load the rskr forarbete rank from YAML."""
    rank = load_yaml(config_path).get("forarbete_types", {}).get("rskr", {}).get("rank")
    if not isinstance(rank, int):
        raise ValueError("forarbete_rank for rskr saknas eller ar inte ett heltal.")
    return rank
...
class RskrBulkIndexer:
    """Read raw rskr JSON, parse, chunk, embed and upsert to Chroma."""
    ...
    def run(
        ...
        *,
        ...
        """Execute the full pipeline over all raw rskr files."""
    ...
    def prepare_document(
        ...
        *,
        ...
        """Prepare one raw rskr document for indexing."""
...
def main(argv: list[str] | None = None) -> int:
    """CLI entrypoint."""
    parser = argparse.ArgumentParser(description="Bulk-indexera rskr till ChromaDB")
